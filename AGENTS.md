...
# Specialized agent team for python · 61 nodes.
...
- Never modify god nodes without declaring in task description: `_buildContext()` (npm/cli.js, 14 edges), `MetaController` (tools/meta_controller.py, 5 edges), `files_in_diff()` (tools/validator.py, 4 edges)
...
Graph source: `graphify-out/GRAPH_REPORT.md` · 61 nodes · 85 edges · last run 2026-05-14

| Node | Edges | File | Owner | Notes |
|------|-------|------|-------|-------|
| `_buildContext()` | 14 | `npm/cli.js` | — | |
...
`_buildContext()` (npm/cli.js) has 14 edges — changes cascade across the graph.
