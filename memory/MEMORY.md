...
| Date | Commit | What |
|------|--------|------|
| 2026-05-24 | 0d3e0ad | BP-2: per-task ROUTE protocol added to templates/CLAUDE.md + both dispatcher prompts (live + template) — ROUTE block required before every task, not just session start; BP-3: Rule 0 behavioral pre-flight added to both validator prompts (non-blocking, conversation-context check); BP-4: `files_touched_in_diff()` added to tools/validator.py using `+++ b/` headers (catches new files); `added_lines_for_file()` file-scoped helper added; both exposed via `_load_project_rules()`; `_rule_mobile_desktop_parity()` switched to `files_touched_in_diff()`; BP-8: templates/bin/commit pytest gate added (step 3, skips if tests/ absent), `refresh_god_nodes()` step added (step 8, no-op if script absent); BP-9: JSX build gate watches `*.jsx\|*.tsx` instead of hardcoded paths; bump 1.0.29 → 1.0.30 |
| 2026-05-15 | 166cb5e | AGENTS.md: date bump to 2026-05-15; README.md: god node stat replaced with placeholder "x edges across x communities" (WIP); docs/examples.md: ⏳ manual steps reformatted as numbered Step 1/Step 2 blocks with expanded guidance text |
